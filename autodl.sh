...
{
    if [[ ! -d ~/www/$(whoami).$(hostname -f)/public_html/rutorrent ]] # check for rutorrent - close if non-existent
    then
    ...
    case "$CHOICE" in
        "1") # install autodl
            ...
            then
                ...
                cp ~/.autodl/autodl.cfg ~/.autodl/autodl.cfg.bak-$(date -I)
            fi
            ...
            rm -rf ~/www/$(whoami).$(hostname -f)/public_html/rutorrent/plugins/autodl-*
            unzip -qo ~/autodl-rutorrent.zip -d ~/www/$(whoami).$(hostname -f)/public_html/rutorrent/plugins/
            mv ~/www/$(whoami).$(hostname -f)/public_html/rutorrent/plugins/autodl-rutorrent-master ~/www/$(whoami).$(hostname -f)/public_html/rutorrent/plugins/autodl-irssi
        ...
        "4") # uninstall autodl irssi
            ...
            then
                ...
                rm -rf ~/.autodl ~/.irssi ~/www/$(whoami).$(hostname -f)/public_html/rutorrent/plugins/autodl-irssi # remove directories
